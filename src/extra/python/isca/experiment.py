...
class Experiment(Logger, EventEmitter):
    ...
    def __init__(
        ...
        self.workdir = workbase.joinpath("experiment", self.name)
        self.rundir = self.workdir.joinpath(
            "run"
        )  # temporary area an individual run will be performed
        self.datadir = database.joinpath(
            self.name
        )  # where run data will be moved to upon completion
        self.restartdir = self.datadir.joinpath(
            "restarts"
        )  # where restarts will be stored
        self.template_dir = _module_directory.joinpath("templates")
        ...
        self.field_table_file = self.codebase.srcdir.joinpath(
            "extra", "model", self.codebase.name, "field_table"
        )
        self.inputfiles = []
    ...
    def get_restart_file(self, i):
        return self.restartdir.joinpath(self.restartfmt % i)
    ...
    def run(
        ...
        if use_restart:
            ...
            if not os.path.isfile(restart_file):
                self.log.error(
                    "Restart file not found, expecting file %r" % restart_file
                )
                raise IOError(
                    "Restart file not found, expecting file %r" % restart_file
                )
        ...
        try:
            ...
            proc = sh.bash(
                P(self.rundir, "run.sh"), _bg=True, _out=_outhandler, _err_to_out=True
            )
